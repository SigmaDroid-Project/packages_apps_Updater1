...
updates_dir=/data/sigma_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have SigmaDroid-VERSION-DATE-TYPE-* as format"
...
# Assume sigmadroid-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.sigma.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.sigma.updater/databases/updates.db" \
